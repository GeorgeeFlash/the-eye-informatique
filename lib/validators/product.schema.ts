...
export const productSchema = productSchemaBase.superRefine((data, ctx) => {
  if (data.commissionType && !data.commissionValue) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Commission value is required when a type is selected", path: ["commissionValue"] })
  }
  if (data.commissionType === "PERCENTAGE" && data.commissionValue && data.commissionValue > 100) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Percentage cannot exceed 100", path: ["commissionValue"] })
  }
})
